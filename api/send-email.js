...
export default async function handler(req, res) {
  ...
  if (!process.env.RESEND_API_KEY) {
    return res.status(500).json({
      error:
        "Ve Vercelu chybí proměnná RESEND_API_KEY."
    });
  }
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "Chybí údaje rezervace nebo je neplatný e-mail."
    });
  }
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden">
      ...
      <div style="padding:28px;color:#1f2937">
        ...
        <p>
          ...
          Vaši rezervaci jsme přijali a nyní čeká na potvrzení restaurací.
        </p>
  ...
  try {
    const customerResult =
      await sendResendEmail({
        to: cleanEmail,
        subject:
          `Rezervace na ${safe.date} v ${safe.time}`,
        html: customerHtml
      });
    ...
    if (restaurantEmail) {
      restaurantResult =
        await sendResendEmail({
          to: restaurantEmail,
          subject:
            `Nová rezervace – ${safe.name}, ${safe.date} ${safe.time}`,
          html: restaurantHtml,
          replyTo: cleanEmail
        });
    }
    ...
    return res.status(200).json({
      ...
      restaurantEmailId:
        restaurantResult?.id || null,
      restaurantNotificationSkipped:
        !restaurantEmail
    });
  } catch (error) {
    console.error(
